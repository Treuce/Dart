...
namespace DaRT
{
	public partial class GUImain : Form
	{
		...
		private void InitializePlayerList()
		{
			...
			playerList.Columns.Add("UID", 100);
		...
		private void InitializePlayerDBList()
		{
			...
			(playerDBContextMenu.Items[0] as ToolStripMenuItem).DropDownItems.Add("UID", null, playerDBCopyUID_click);
		...
		{
			if (!(AdminsTab.SelectedTab.Text == "Player Database"))
			{
		...
		private void playerCopyUID_click(object sender, EventArgs args)
		{
			// Copying GUID to clipboard
			try
			{
		...
		private void playerDBCopyUID_click(object sender, EventArgs args)
		{
			// Copying GUID to clipboard
			try
			{
			...
			try
			{
				...
				if (players != null)
				{
					...
					for (int i = 0; i < players.Count; i++)
					{
						...
						String[] entries = { "", players[i].number.ToString(), players[i].ip, players[i].ping, players[i].guid, players[i].uid, players[i].name, players[i].status, comment };
								...
								using (SqliteCommand selectCommand = new SqliteCommand("SELECT id, guid, name FROM players WHERE guid = @guid AND name = @name LIMIT 0, 1", connection))
								{
									...
									using (SqliteDataReader reader = selectCommand.ExecuteReader())
									{
										if (!reader.Read())
										{
											...
											{
												using (SqliteCommand addCommand = new SqliteCommand("INSERT INTO players (id, lastip, lastseen, guid, name, lastseenon, synced, uid) VALUES(NULL, @lastip, @lastseen, @guid, @name, @lastseenon, 0, @uid)", connection))
												{
													addCommand.Parameters.Clear();
													...
													addCommand.Parameters.Add(new SqliteParameter("@uid", uid));
													addCommand.ExecuteNonQuery();
												}
											}
										}
		...
		public void thread_Database()
		{
			try
			{
				...
				List<Player> playersDB = new List<Player>();
				using (command = new SqliteCommand("SELECT id, lastip, lastseen, guid, name, lastseenon,uid FROM players ORDER BY id ASC", connection))
				{
					using (SqliteDataReader reader = command.ExecuteReader())
					{
						while (reader.Read())
		...
		private void thread_Sync()
		{
			...
			try
			{
				...
				command = new SqliteCommand(connection);
				command.CommandText = "SELECT guid, uid, name, lastip, lastseenon, location, lastseen FROM players WHERE synced = 0";
				...
				while (reader.Read())
				{
					...
					string uid = GetSafeString(reader, 1);
					String name = this.GetSafeString(reader, 2);
					String lastip = this.GetSafeString(reader, 3);
					String lastseenon = this.GetSafeString(reader, 4);
